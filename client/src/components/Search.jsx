  ...
  const projectFilter =
    fetch === ""
      ...
      : projects.filter((project) =>
          project.name.toLowerCase().includes(fetch.toLowerCase())
        );
    ...
    >
      ...
      >
        ...
        >
          <Combobox
            as="div"
            className="mx-auto max-w-xl transform divide-y divide-gray-100 overflow-hidden  rounded bg-white shadow-2xl ring-1 ring-black ring-opacity-5 transition-all"
            onChange={(project) =>
              (window.location = `/projetos/${project._id}`)
            }
          >
